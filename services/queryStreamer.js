...
const campaignExecutionDbConfig = require("../config/db-config.js")[env];
...
async function connectDb(config) {
  try {
    ...
    error.code = "SMART_QUEQE_MANAGER_DB_CONNECTION_ERROR";
    const connection = {};
...
async function queryStreamer(sql, sqlArgs = [], logger) {
  const config = campaignExecutionDbConfig;
